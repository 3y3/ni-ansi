...
WebInspector.ANSIFormatter = module.exports;

WebInspector.ConsoleViewEventDispatcher.addEventListener(
  WebInspector.ConsoleViewEventDispatcher.Events.MessageFormatted,
  function(event) {
    var formattedMessage = event.data;

    if (!formattedMessage.classList.contains('console-message-text')) return;

    var spans = Array.prototype.slice.call(formattedMessage.querySelectorAll('span'));
    spans.forEach(function(span) {
      span.innerHTML = new WebInspector.ANSIFormatter().toHtml(span.innerHTML);
    });
  }
);
